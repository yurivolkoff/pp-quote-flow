...
5. **The location control is production's; its option values are not.** R3b applied the captured tumbler's list flow-wide. **Reversed in R5** — imprint locations are product data, so the pen keeps Barrel / Clip / Cap and gains production's per-product `No Imprint` opt-out. What carries over from production is the combobox idiom, the label formatting, the step names and the reset-on-method-change behaviour. See Decisions item 1.
...
1. **Imprint locations are product data, not UI vocabulary — the rule, so it is not re-applied wrongly.** What production owns and the flow must follow is the *control idiom* (a combobox, never a swatch group), the *label formatting*, the *step names*, and the *reset-to-the-first-option on method change*. What production does **not** own is the option values: those come from the product. The pen therefore offers Barrel / Clip / Cap / No Imprint, not the captured tumbler's Back / Front (small) / Front (large) / Two Sided. R3b briefly applied the tumbler's values flow-wide; that was wrong and is reversed in R5. The same rule holds for method names as *values* — Pad Printed is the pen's own data, rendered in production's Title Case participle *format*.
2. **C has no pinned footer.** The analysis specced a pinned footer for D only. C runs under 500px tall, so its CTA sits in flow; at 375 it lands just at the fold rather than above it, which is the one place C is slightly worse than the inline panel the analysis preferred.
3. **One breakdown string across the flow.** Production ships both "See price breakdown" (apron) and "View Price Breakdown" (tumbler) for the same control. The flow uses `View Price Breakdown` everywhere, so C, D and the PDP agree. A Boris ask asks production to normalise. **Deck correction:** the R3 copy-deck delta logged `C-06` "See price breakdown" as "production string verbatim". That provenance claim is wrong — it came from the apron page, and the configurator capture shows the button reads "View Price Breakdown". The deck entry should be corrected at source.
4. **The method card carries a one-line constraint summary** ("Up to 4 colors · 5 locations"). Production's card is name-only, because the constraints live in a Decoration & Imprint accordion that D does not have. Without it, D's clamp notices arrive unexplained.
5. **The colour-count clamp above one colour needed a second string.** `D-21` is written for a method that prints in one colour. A method that clamps from four colours to three gets a parallel sentence in the same shape.
6. **`No Imprint` is a real priced state.** Choosing it drops the setup and run charges to zero, sets `Imprint Colors` to `None`, and removes those lines from the breakdown, rather than quoting a decoration the buyer just declined.
...
9. **The email carries the full breakdown**, not a short summary block, because the instruction was that the production breakdown lines appear on the quote page, the PDF and the email alike.
10. **The tier curve, the per-method charges, the rep contact details and the product image** are mock, carried from the R1 build. The location list and the method names are no longer mock: both are production's.
...
3. **Whether the two modules should be open to any token holder.** Built open, per the analysis: the same token already authorises "Order this quote", so gating a phone number would be incoherent. Edits are date-stamped.
...
- 2026-09-03 **R5 — imprint locations restored to product data.** The pen offers Barrel / Clip / Cap / No Imprint again, in D's combobox and everywhere the configuration is summarised: C's line, the quote page, the PDF and the email. Production still owns the control idiom, the label formatting, the step names and the reset-to-first-option on method change; it does not own the option values. The rule is written into Decisions item 1 so the tumbler's values are not re-applied later. R4 items 6, 7 and 8 re-verified unchanged after the swap.
